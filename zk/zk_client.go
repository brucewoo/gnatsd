...
func Connect(servers []string, recvTimeout time.Duration) (*ZkClient, <-chan Event, error) {
	return ConnectWithDialer(servers, recvTimeout, nil)
}

func ConnectWithDialer(servers []string, recvTimeout time.Duration, dialer Dialer) (*ZkClient, <-chan Event, error) {
...
func (c *ZkClient) State() State {
	return State(atomic.LoadInt32((*int32)(&c.state)))
}

func (c *ZkClient) setState(state State) {
...
func (c *ZkClient) connect() bool {
	...
	for {
		conn, err := c.dialer("tcp", c.servers[c.serverIndex], c.connectTimeout)
		if err == nil {
			c.conn = conn
...
func (c *ZkClient) loop() {
	for {
		if !c.connect() {
			log.Printf("connect to zookeeper failed.")
			return
		}
		err := c.authenticate()
		switch {
...
func (c *ZkClient) flushRequests(err error) {
	c.requestsLock.Lock()
...
func (c *ZkClient) invalidateWatches(err error) {
	c.watchersLock.Lock()
...
func (c *ZkClient) sendSetWatches() {
	c.watchersLock.Lock()
	defer c.watchersLock.Unlock()
...
func (c *ZkClient) authenticate() error {
	buf := make([]byte, 256)
...
func (c *ZkClient) sendLoop(conn net.Conn, closeChan <-chan bool) error {
...
func (c *ZkClient) recvLoop(conn net.Conn) error {
	buf := make([]byte, bufferSize)
	for {
		// package length
...
func (c *ZkClient) nextXid() int32 {
	return atomic.AddInt32(&c.xid, 1)
}

func (c *ZkClient) addWatcher(path string, watchType watchType) <-chan Event {
	c.watchersLock.Lock()
	defer c.watchersLock.Unlock()
...
func (c *ZkClient) queueRequest(opcode int32, req interface{}, res interface{}, recvFunc func(*request, *responseHeader, error)) <-chan response {
	rq := &request{
		xid:        c.nextXid(),
		opcode:     opcode,
		pkt:        req,
...
func (c *ZkClient) request(opcode int32, req interface{}, res interface{}, recvFunc func(*request, *responseHeader, error)) (int64, error) {
	r := <-c.queueRequest(opcode, req, res, recvFunc)
	return r.zxid, r.err
}

func (c *ZkClient) AddAuth(scheme string, auth []byte) error {
	_, err := c.request(opSetAuth, &setAuthRequest{Type: 0, Scheme: scheme, Auth: auth}, &setAuthResponse{}, nil)
	return err
}

func (c *ZkClient) Children(path string) ([]string, *Stat, error) {
	res := &getChildren2Response{}
	_, err := c.request(opGetChildren2, &getChildren2Request{Path: path, Watch: false}, res, nil)
	return res.Children, &res.Stat, err
}

func (c *ZkClient) ChildrenW(path string) ([]string, *Stat, <-chan Event, error) {
	var ech <-chan Event
	res := &getChildren2Response{}
...
func (c *ZkClient) GetW(path string) ([]byte, *Stat, <-chan Event, error) {
	var ech <-chan Event
	res := &getDataResponse{}
...
func (c *ZkClient) Set(path string, data []byte, version int32) (*Stat, error) {
	res := &setDataResponse{}
	_, err := c.request(opSetData, &SetDataRequest{path, data, version}, res, nil)
	return &res.Stat, err
}

func (c *ZkClient) Create(path string, data []byte, flags int32, acl []ACL) (string, error) {
	res := &createResponse{}
...
// create exists.
func (c *ZkClient) CreateProtectedEphemeralSequential(path string, data []byte, acl []ACL) (string, error) {
	var guid [16]byte
...
func (c *ZkClient) Exists(path string) (bool, *Stat, error) {
	res := &existsResponse{}
...
func (c *ZkClient) ExistsW(path string) (bool, *Stat, <-chan Event, error) {
	var ech <-chan Event
	res := &existsResponse{}
...
func (c *ZkClient) GetACL(path string) ([]ACL, *Stat, error) {
	res := &getAclResponse{}
	_, err := c.request(opGetAcl, &getAclRequest{Path: path}, res, nil)
	return res.Acl, &res.Stat, err
}

func (c *ZkClient) SetACL(path string, acl []ACL, version int32) (*Stat, error) {
	res := &setAclResponse{}
	_, err := c.request(opSetAcl, &setAclRequest{Path: path, Acl: acl, Version: version}, res, nil)
	return &res.Stat, err
}

func (c *ZkClient) Sync(path string) (string, error) {
	res := &syncResponse{}
...
func (c *ZkClient) Multi(ops MultiOps) error {
	req := &multiRequest{
